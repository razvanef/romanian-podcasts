    ...
    <v-dialog v-model="addPodcastDialog" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <v-text-field
                  v-model="podcast.googlePodcasts"
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="addPodcast(podcast)"
            >Adauga</v-btn
          >
        </v-card-actions>
      </v-card>
  ...
  methods: {
    addPodcast(podcast) {
      fetch(podcast.rss)
        ...
        .then(data => {
          console.log(data);
          const name = data.getElementsByTagName("title")[0].childNodes[0]
            .nodeValue;
          const podcastId = name
            .toLowerCase()
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/[^a-zA-Z ]/g, "")
          ...
          const website = data.getElementsByTagName("link")[0].childNodes[0]
            .nodeValue;
          const host = data.getElementsByTagName("author")[0]
            ? data.getElementsByTagName("author")[0].childNodes[0].nodeValue
            : data.getElementsByTagName("itunes:author")[0].childNodes[0]
                .nodeValue;
          const cover = data
            .getElementsByTagName("image")[0]
            .getElementsByTagName("url")[0].childNodes[0].nodeValue;
          const categories = Array.from(
            data.getElementsByTagName("itunes:category")
          )
